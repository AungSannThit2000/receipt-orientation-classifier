...
The application declares `opencv-python-headless` for server use. Ultralytics also installs `opencv-python` transitively, so `packages.txt` installs the Linux `libgl1` and Debian 13 `libglib2.0-0t64` runtime libraries required when that OpenCV wheel is imported on Streamlit Community Cloud.
